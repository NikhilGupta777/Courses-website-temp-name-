// Fix #1: replaced the broken custom line-by-line renderer (which silently
//         dropped table rows, code fences, and inline code content) with
//         react-markdown + remark-gfm.  The old renderer had comments saying
//         "Table rows handled below" and "Code blocks handled below" but there
//         was no such code — all that content simply vanished at render time.
//
// Fix #2: the prose-* Tailwind classes require @tailwindcss/typography.
//         Added the plugin to package.json (devDependencies) and activated it
//         in globals.css via `@plugin "@tailwindcss/typography"`.  The
//         className below is unchanged so the visual design is preserved.
//
// This is a Server Component (no "use client") — react-markdown works fine
// server-side and generateMetadata is only available in Server Components.
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-xs text-white/70">
                {post.author.role} · {post.publishedAt} · {post.readingTime}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          ...
          <article className="lg:col-span-3">
            ...
            {/*
              Fix #1 + #2:
              - react-markdown + remark-gfm renders tables, code fences, inline
                code, blockquotes, and all other GFM syntax correctly.
              - prose-* classes now work because @tailwindcss/typography is
                installed (devDep) and activated via globals.css @plugin.
            */}
            <div className="prose prose-lg max-w-none prose-headings:font-bold prose-headings:text-gray-900 prose-p:text-gray-600 prose-p:leading-relaxed prose-a:text-violet-600 prose-a:no-underline hover:prose-a:underline prose-code:bg-gray-100 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-sm prose-pre:bg-gray-900 prose-pre:rounded-2xl prose-blockquote:border-l-violet-500 prose-blockquote:bg-violet-50 prose-blockquote:not-italic prose-table:text-sm">
            ...
            <div className="mt-12 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-8 text-white text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <Link
                  href="/register"
                  className="px-6 py-3 bg-white/10 text-white font-semibold rounded-xl border border-white/30 hover:bg-white/20 transition-colors text-sm"
                >
                  Start Free
                </Link>
              </div>
                  ...
                  {[
                    {
                      label: "Twitter",
                      bg:    "bg-sky-500",
                      href:  `https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`https://learnai.in/blog/${post.slug}`)}`,
                    },
                    {
                      label: "LinkedIn",
                      bg:    "bg-blue-700",
                      href:  `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://learnai.in/blog/${post.slug}`)}`,
                    },
                  ].map((s) => (
                    <a
                      key={s.label}
                      ...
                      <Link key={rel.slug} href={`/blog/${rel.slug}`} className="block group">
                        <div className="flex gap-3 items-start">
                          ...
                          <div className="min-w-0">
                            <div className="text-xs font-semibold text-gray-900 group-hover:text-violet-600 transition-colors line-clamp-2">
                              {rel.title}
                            </div>
