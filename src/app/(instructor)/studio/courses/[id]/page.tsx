      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {course.status === "DRAFT" && (
              <button onClick={() => publishCourse.mutate({ id })} disabled={publishCourse.isPending}
        ...
        <div className="flex gap-1 bg-gray-100 rounded-2xl p-1.5 mb-6 w-fit">
          {(["info", "curriculum", "pricing"] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-xl text-sm font-medium capitalize transition-all ${activeTab === tab ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
              {tab}
            </button>
          ))}
        </div>
        ...
        {activeTab === "curriculum" && (
          <div className="space-y-4">
            {(course.modules ?? []).map((mod, mi) => (
              <div key={mod.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="flex items-center gap-3 px-5 py-4 bg-gray-50 border-b border-gray-100">
                  <div className="w-7 h-7 rounded-lg bg-violet-100 text-violet-700 text-xs font-bold flex items-center justify-center flex-shrink-0">{mi + 1}</div>
                  ...
                  <button onClick={() => deleteModule.mutate({ id: mod.id })}
                ...
                <div className="divide-y divide-gray-50">
                  {mod.lessons.map((lesson) => (
                    <div key={lesson.id} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/50">
                      ...
                      {lesson.isFree && <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded">Free</span>}
                      <button onClick={() => deleteLesson.mutate({ id: lesson.id })}
                ...
                <div className="px-5 py-3 border-t border-gray-50">
                  <button onClick={() => createLesson.mutate({ moduleId: mod.id, title: "New Lesson", type: "VIDEO" })}
            ...
            <button onClick={() => createModule.mutate({ courseId: id, title: "New Module" })}
